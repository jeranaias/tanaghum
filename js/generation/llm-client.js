...
IMPORTANT: This transcript was auto-generated by speech recognition and may contain spelling errors. Always use the CORRECT Arabic spelling for each word, not the misspelled version. For example if the transcript says "شطيران" you should write "طيران", if it says "مناطسي" write "مغناطيسي", if it says "أرثام" write "أرقام".

ROOT GUIDELINES:
- Arabic roots are typically 3 consonant letters (trilateral), sometimes 4 (quadrilateral)
- Write roots using only the root consonants: e.g. ق-ت-ل for مقاتل, ط-ي-ر for طائرات
- Do NOT include prefixes like م or ت in the root
- For loanwords (تكنولوجيا, استراتيجية, etc.) set root to "—"

EXAMPLE OUTPUT FORMAT:
[
  {"word_ar": "زلزال", "word_en": "earthquake", "root": "ز-ل-ز-ل", "pos": "noun", "definition_en": "a shaking of the ground", "frequency": "medium"},
  {"word_ar": "طائرات", "word_en": "aircraft", "root": "ط-ي-ر", "pos": "noun", "definition_en": "flying machines", "frequency": "high"},
  {"word_ar": "تكنولوجيا", "word_en": "technology", "root": "—", "pos": "noun", "definition_en": "the application of scientific knowledge", "frequency": "high"}
]

Required fields for each word:
- word_ar: the CORRECT Arabic spelling (fix any transcription errors)
- word_en: English translation
- root: Arabic root consonants with dashes (or "—" for loanwords)
- pos: part of speech
...
OUTPUT (JSON array only, no other text):`;
    ...
    // Post-process: validate and clean vocabulary entries
    vocabulary = vocabulary.filter(item => {
      if (!item || !item.word_ar) return false;
      // Filter out entries with no Arabic characters
      if (!/[\u0600-\u06FF]/.test(item.word_ar)) return false;
      return true;
    }).map(item => {
      // Validate root: should be 2-4 consonants separated by dashes, or "—" for loanwords
      if (item.root && item.root !== '—') {
        const rootParts = item.root.split('-').filter(p => p.trim());
        if (rootParts.length > 4) {
          // Root too long (e.g. letter-by-letter loanword) — mark as loanword
          log.warn(`Root too long for "${item.word_ar}": "${item.root}", marking as loanword`);
          item.root = '—';
        }
      }
      return item;
